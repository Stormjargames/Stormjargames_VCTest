...
[CustomEditor(typeof(PieceTemplateSO))]
public class DrawPieceTemplateSO : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        //Cast target to PieceTemplateSO
        PieceTemplateSO m = (PieceTemplateSO)target;
        ...
        GUILayout.BeginHorizontal();
        m.direction = (CompassPoints)EditorGUILayout.EnumPopup(m.direction);
        GUILayout.EndHorizontal();
        GUILayout.BeginVertical();
        EditorGUILayout.LabelField("Disable");
        m.disable = EditorGUILayout.Toggle(m.disable);
        EditorGUILayout.LabelField("Boost when tied");
        m.boostWhenTied = EditorGUILayout.Toggle(m.boostWhenTied);
    ...
    void CheckArraySizes(PieceTemplateSO m)
    {
        if (m.myThings == null ||
            m.myThings.Length == 0 ||
            m.myThings[0] == null ||
            m.myThings[0].entries.Length == 0)
        {
            ...
            m.myThings = new PieceTemplateSO.Row[m.height];
            for (int i = 0; i < m.myThings.Length; i++)
            {
                m.myThings[i] = new PieceTemplateSO.Row();
